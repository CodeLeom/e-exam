                        ...
                        <div class="containe" style="margin-top: 15px">
                            <?php
                                ...
                                while($c_row = mysqli_fetch_array($select_course_query)) {
                                    ...
                                    $name = $c_row['name'];
                                    if($_SESSION['exam_type'] == 'P'){
                                        $this_exam_id = getDefiniteExam('id','exams','active','1','subject_id',$subject_id,'year',$year,'type','P');
                                        if(!is_numeric($this_exam_id)){
                                            redirect('error.php?t=e.id.e');
                                        }
                                        $exam_id = $this_exam_id;
                                    }else {
                                        $this_exam_id = getDefiniteExam('id','exams','active','1','subject_id',$subject_id,'year',$year,'type','M');
                                        if(!is_numeric($this_exam_id)){
                                            redirect('error.php?t=e.id.e');
                                        }
                                        $exam_id = $this_exam_id;
                                ...
                                <a href="pre_start.php?xsssdsdxxssdhfdghsjhfjdhdhsjdhdhfjfhsbsdhddjshs=<?php echo $exam_id ?>"" class="btn <?php if($exam_id == $c_exam_id){ echo "btn-danger"; }else{ echo "btn-success"; } ?>"><?php echo "<span class='text-capitalize'>$name</span>"; ?></a>
